...
import { X, MapPin, Clock, Leaf, Star, Users, Calendar, Navigation, DollarSign, TreePine, Car, Building2 } from "lucide-react"
...
export default function ItineraryDetails({ itinerary, onClose }) {
  ...
  const initializeMap = () => {
    ...
    const map = new window.google.maps.Map(mapElement, {
      center: { lat: itinerary.coordinates.lat, lng: itinerary.coordinates.lng },
      zoom: 12,
      styles: [
        {
              ...
              icon: {
                url: 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(`
                  <svg width="28" height="28" viewBox="0 0 28 28" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <text x="14" y="18" text-anchor="middle" fill="white" font-size="10" font-weight="bold">${dayIndex + 1}</text>
                  </svg>
                `),
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-6xl w-full max-h-[95vh] overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white p-6 relative">
          ...
          <h2 className="text-2xl font-bold mb-2">{itinerary.title || `${itinerary.destination} Adventure`}</h2>
        ...
        <div className="overflow-y-auto max-h-[calc(95vh-180px)]">
          <div className="p-6">
            ...
            {activeTab === 'overview' && (
              <div className="space-y-6">
                  ...
                  <div className="bg-blue-50 rounded-lg p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">${itinerary.summary?.accommodationCost || 0}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Activities:</span>
                        <span className="font-medium">${itinerary.summary?.activitiesCost || 0}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Transport:</span>
                        <span className="font-medium">${itinerary.summary?.transportCost || 0}</span>
                      </div>
                  ...
                  <div className="bg-green-50 rounded-lg p-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{itinerary.summary?.totalCarbonFootprint || 0}kg CO₂</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Carbon Saved:</span>
                        <span className="font-medium text-green-600">{itinerary.sustainability?.totalCarbonSaved || 0}kg CO₂</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Eco Activities:</span>
                        <span className="font-medium">{itinerary.sustainability?.ecoFriendlyActivities || 0}/{itinerary.sustainability?.totalActivities || 0}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {itinerary.sustainability?.sustainabilityFeatures && (
                  <div>
                    <h3 className="font-semibold text-gray-900 mb-4">Sustainability Features</h3>
            ...
            {activeTab === 'itinerary' && itinerary.days && (
              <div className="space-y-6">
                <h3 className="font-semibold text-gray-900 mb-4">Daily Itinerary</h3>
                <div className="space-y-6">
                      ...
                      <div className="p-6 space-y-4">
                        {day.activities.map((activity, idx) => (
                          <div key={idx} className="flex gap-4 p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                            <div className="text-sm font-semibold text-blue-600 min-w-[70px] bg-blue-100 px-2 py-1 rounded">
                              {activity.time}
                            </div>
                            <div className="flex-1">
                              ...
                              <div className="flex flex-wrap gap-1">
                                {activity.ecoFeatures?.map((feature, featureIdx) => (
                                  <span key={featureIdx} className="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full">
                                    {feature}
                                  </span>
                                ))}
                              </div>
            ...
            {/* Accommodations Tab */}
            {activeTab === 'accommodations' && itinerary.accommodations && itinerary.accommodations.length > 0 && (
              <div className="space-y-6">
                <h3 className="font-semibold text-gray-900 mb-4">Eco-Friendly Accommodations</h3>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {itinerary.accommodations.map((hotel) => (
                    <div key={hotel.id} className="border border-gray-200 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                      {hotel.image && (
                      ...
                      <div className="p-6">
                        <div className="flex items-start justify-between mb-3">
                          <div>
                            <h4 className="font-semibold text-gray-900 text-lg mb-1">{hotel.name}</h4>
                        ...
                        <div className="space-y-3">
                          <div>
                            <h5 className="text-sm font-medium text-gray-900 mb-2">Eco Features</h5>
                            <div className="flex flex-wrap gap-1">
                          ...
                          <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                            <div className="text-sm">
                              ...
                              <span className="font-medium">{hotel.distance?.value} {hotel.distance?.unit}</span>
                            </div>
                            <div className="text-sm">
                              <span className="text-gray-600">Carbon footprint: </span>
                              <span className="font-medium text-green-600">{hotel.carbonFootprint}kg CO₂</span>
                            </div>
                          </div>
                        </div>
                      </div>
            ...
            {activeTab === 'transport' && itinerary.transport && itinerary.transport.length > 0 && (
              <div className="space-y-6">
                <h3 className="font-semibold text-gray-900 mb-4">Sustainable Transport Options</h3>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {itinerary.transport.map((transport, idx) => (
                    <div key={idx} className="border border-gray-200 rounded-lg p-6 hover:shadow-md transition-shadow">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <div className="flex-1">
                          <h4 className="font-semibold text-gray-900 text-lg">{transport.name}</h4>
                          <p className="text-sm text-gray-600">{transport.availability}</p>
                          <p className="text-xs text-gray-500 mt-1">{transport.description}</p>
                        </div>
                        <div className="text-right">
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                          <span className="text-sm text-green-800">Carbon Savings</span>
                          <span className="font-semibold text-green-600">{transport.carbonSavings}kg CO₂</span>
                        </div>
                        
                        <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                          <span className="text-sm text-blue-800">Available Locations</span>
                          <span className="font-semibold text-blue-600">{transport.count} stations</span>
                        </div>
                        
                        <div className="p-3 bg-gray-50 rounded-lg">
                          <p className="text-sm text-gray-700 mb-2">
                            <span className="font-medium">Recommendation:</span> {transport.recommendation}
                          </p>
                          <div className="text-xs text-gray-600">
                            Carbon Factor: {transport.carbonFactor}kg CO₂ per km
                          </div>
                        </div>
                        
                        {transport.places && transport.places.length > 0 && (
                          <div>
                            <h5 className="text-sm font-medium text-gray-900 mb-2">Nearby Stations</h5>
                            <div className="space-y-1 max-h-32 overflow-y-auto">
                              {transport.places.slice(0, 3).map((place, placeIdx) => (
                                <div key={placeIdx} className="text-xs text-gray-600 flex items-center gap-1">
                                  <MapPin size={10} />
                                  <span>{place.name || place.vicinity || 'Station'}</span>
                                </div>
                              ))}
                            </div>
                          </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  <div className="bg-emerald-50 p-4 rounded-lg">
                    <h4 className="font-medium text-emerald-800 mb-2">🏨 Accommodations</h4>
                    <div className="space-y-1 text-sm">
                      {itinerary.accommodations?.slice(0, 3).map((hotel, idx) => (
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg">
                    <h4 className="font-medium text-blue-800 mb-2">📍 Daily Activities</h4>
                    <div className="space-y-1 text-sm">

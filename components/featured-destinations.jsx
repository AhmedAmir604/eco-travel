        ...
        for (const city of featuredCities) {
          try {
            const response = await fetch(
              `/api/activities?city=${encodeURIComponent(city.name)}&latitude=${city.coords.latitude}&longitude=${city.coords.longitude}&radius=10`
            )
            const data = await response.json()
            
            if (data.success && data.data) {
              // Take first 2 activities from each city
              const cityActivities = data.data.slice(0, 2).map(activity => ({
                ...activity,
                cityName: city.name
              }))
            ...
            console.warn(`Failed to load activities for ${city.name}:`, cityError)
          }
        }
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 max-w-2xl mx-auto">
            Discover sustainable activities from around the world that offer unforgettable experiences while supporting
            environmental conservation and local communities.
          </p>
        </div>
        ...
        {error && !loading && (
          <div className="text-center py-12">
            <div className="text-red-500 mb-4">
              <p className="text-lg font-medium mb-2">Unable to load featured activities</p>
        ...
        {!loading && !error && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {activities.map((activity) => (
              <Link href={`/activities/${activity.id}`} key={activity.id} className="group">
                <div className="bg-white rounded-lg shadow-md overflow-hidden transition-transform group-hover:shadow-lg hover:scale-105">
                  <div className="relative h-48 w-full">
                    ...
                    <div className="absolute top-2 right-2 bg-white bg-opacity-90 px-2 py-1 rounded-full text-xs font-medium">
                      {activity.price ? `${activity.price.amount} ${activity.price.currency}` : 'Free'}
                    </div>
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      {activity.rating ? (
                        ...
                        <span className="text-sm text-green-600 font-medium">Eco Activity</span>
                      )}
        ...
        {!loading && !error && activities.length === 0 && (
          <div className="text-center py-12">
            <div className="text-gray-500 mb-4">
              <h3 className="text-lg font-medium mb-2">No featured activities available</h3>

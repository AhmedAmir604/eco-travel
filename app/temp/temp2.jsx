...
export default function ItineraryPage() {
  ...
  const loadItineraryData = async () => {
    ...
    try {
      // Fetch the actual itinerary data from API
      const response = await fetch(`/api/itinerary/${params.id}`)
      const result = await response.json()

      if (result.success) {
        console.log('Loaded itinerary data:', result.data)
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="text-gray-400 mb-4">
            ...
            <p className="text-gray-500 mb-4">The requested itinerary could not be found or may have expired.</p>
          </div>
          <Link
            href="/itineraries"
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div className="text-center">
                  <div className="text-4xl font-bold text-emerald-600 mb-2">
                    {Math.round((itinerary.sustainability?.totalCarbonSaved || itinerary.summary?.carbonSaved || 0) * 10) / 10}kg
                  </div>
                  <div className="text-sm text-gray-600">CO₂ Saved</div>
                </div>
                <div className="text-center">
                  <div className="text-4xl font-bold text-emerald-600 mb-2">
                    {itinerary.sustainability?.ecoScore || itinerary.summary?.sustainabilityScore || 4.5}/5
                  </div>
                ...
                <div className="text-center">
                  <div className="text-4xl font-bold text-emerald-600 mb-2">
                    {itinerary.duration || itinerary.days?.length || 0}
                  </div>
              ...
              <div className="bg-green-50 rounded-xl p-6">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center p-3 bg-white rounded-lg">
                    ...
                    <span className="font-semibold text-lg">{Math.round((itinerary.summary?.totalCarbonFootprint || itinerary.days?.reduce((total, day) => total + (day.carbonFootprint || 0), 0) || 0) * 10) / 10}kg CO₂</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-white rounded-lg">
                    <span className="text-gray-700">Carbon Saved:</span>
                    <span className="font-semibold text-lg text-green-600">{Math.round((itinerary.sustainability?.totalCarbonSaved || itinerary.summary?.carbonSaved || 0) * 10) / 10}kg CO₂</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-white rounded-lg">
                    <span className="text-gray-700">Eco Activities:</span>
                    <span className="font-semibold text-lg">{itinerary.sustainability?.ecoFriendlyActivities || itinerary.days?.reduce((total, day) => total + (day.activities?.length || 0), 0) || 0}/{itinerary.sustainability?.totalActivities || itinerary.days?.reduce((total, day) => total + (day.activities?.length || 0), 0) || 0}</span>
                  </div>

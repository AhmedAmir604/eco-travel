  ...
  try {
    ...
    try {
      if (typeof location === "string" && !location.includes(",")) {
        ...
        // //(`Geocoded "${location}" to coordinates:`, coordinates)
      } else {
            ...
            const altResults = altFiltered.map((place) => ({
              ...
              photos: place.photos
                ? place.photos.slice(0, 3).map((photo) => ({
                    ...
                    width: photo.width,
                    height: photo.height,
                    url: getPlacePhotoUrl(photo.photo_reference, 800, 600), // High quality
...
export function getPlacePhotoUrl(
  ...
  const width = Math.min(maxWidth, 1600);
  const height = Math.min(maxHeight, 1600);
      ...
      if (place.photos && place.photos.length > 0) {
        return {
          ...
          highRes: place.photos.slice(0, 3).map(
            (photo) => getPlacePhotoUrl(photo.photo_reference, 1600, 1200) // Maximum quality for detail views
          ),
        };
      }
    }

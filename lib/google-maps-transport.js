...
// 🚀 OPTIMIZATION: Smart location parser - avoids redundant geocoding
// Accepts: "lat,lng" string, {lat, lng} object, or address string
// Returns: "lat,lng" string or null
async function parseLocation(location) {
  try {
    // Case 1: Already a coordinate string "lat,lng"
    if (
      typeof location === "string" &&
      /^-?\d+\.?\d*,-?\d+\.?\d*$/.test(location.trim())
    ) {
      console.log(`✓ Using provided coordinates: ${location}`);
      return location.trim();
    }

    // Case 2: Coordinate object {lat, lng} or {latitude, longitude}
    if (typeof location === "object" && location !== null) {
      const lat = location.lat || location.latitude;
      const lng = location.lng || location.longitude;
      if (lat !== undefined && lng !== undefined) {
        const coords = `${lat},${lng}`;
        console.log(`✓ Using provided coordinate object: ${coords}`);
        return coords;
      }
    }

    // Case 3: Address string - need to geocode (only as last resort)
    if (typeof location === "string") {
      console.log(`🔍 Geocoding address: ${location}`);
      const geocoded = await geocodeAddress(location);
      const coords = `${geocoded.location.lat},${geocoded.location.lng}`;
...
// 🚀 OPTIMIZATION: Accepts coordinates directly to avoid redundant geocoding
export async function discoverAreaTransport(location, radius = 5000) {
  try {
    ...
    // //(`🔍 Searching ${Object.keys(TRANSPORT_TYPES).length} transport types in parallel...`)

    const searchPromises = Object.entries(TRANSPORT_TYPES).map(
      async ([placeType, config]) => {
        try {
          const places = await findNearbyPlaces(coordinates, placeType, radius);

          if (places.length > 0) {
            // //(`✅ Found ${places.length} ${placeType} locations`)

            // Get the best image from the first place with photos
            let mainImage = null;
            const placeWithPhoto = places.find(
...
function getEcoRecommendation(route, mode) {
  ...
  switch (mode) {
    case "WALKING":
      if (distance < 2)
        return "Perfect for short trips! Zero emissions and great exercise.";
      if (distance < 5)
        return "Excellent choice for medium distances. Completely carbon-free!";
      return "Long walk but amazing for the environment and your health!";

    case "BICYCLING":
      if (distance < 5)
        return "Ideal cycling distance! Fast, fun, and zero emissions.";
      if (distance < 15)
        return "Great cycling route! Much faster than walking, still zero carbon.";
      return "Long cycling adventure! Perfect for eco-conscious travelers.";

    case "TRANSIT":
      if (durationValue < 30)
        return "Quick and eco-friendly! Public transit is great for the planet.";
      if (durationValue < 60)
        return "Efficient public transport with low carbon footprint.";
...
function getEcoImpact(carbonFactor) {
  if (carbonFactor === 0)
    return "Zero emissions - Perfect for the environment!";
  if (carbonFactor < 0.05) return "Ultra-low emissions - Excellent eco choice!";
  if (carbonFactor < 0.1)
    return "Low emissions - Great for sustainable travel!";
  if (carbonFactor < 0.2)
    return "Moderate emissions - Good shared transport option!";
...
function getAreaRecommendations(config, count) {
  ...
  if (config.ecoScore === 4) {
    ...
    if (count > 10)
      recommendations.push("Well-connected area with good coverage!");
  }
...
async function findNearbyPlaces(location, type, radius = 3000) {
  try {
    ...
    if (type === "electric_vehicle_charging_station") {
      ...
      //(`Making EV charging station text search: ${MAPS_API_BASE}/place/textsearch/json?${params}`)
      response = await fetch(
        `${MAPS_API_BASE}/place/textsearch/json?${params}`
      );
      ...
      //(`Making transit station text search: ${MAPS_API_BASE}/place/textsearch/json?${params}`)
      response = await fetch(
        `${MAPS_API_BASE}/place/textsearch/json?${params}`
      );
      data = await response.json();
      ...
      //(`Making API request: ${MAPS_API_BASE}/place/nearbysearch/json?${params}`)
      response = await fetch(
        `${MAPS_API_BASE}/place/nearbysearch/json?${params}`
      );
      data = await response.json();
    }

    // //(`API Response for ${type}:`, {
    //   status: data.status,
    ...
    if (!data.results || data.results.length === 0) {
      // //(`No results found for ${type} at ${location}`)
      return [];
    }
    ...
    if (type === "electric_vehicle_charging_station") {
      filteredResults = data.results.filter((place) => {
        ...
        const isEVStation =
          ...
          types.some(
            (type) => type.includes("gas_station") && name.includes("electric")
          );
      ...
      //(`Filtered EV stations: ${filteredResults.length} out of ${data.results.length} results`)
    } else if (type === "transit_station" || type === "bus_station") {
      filteredResults = data.results.filter((place) => {
        const name = place.name.toLowerCase();
        const types = place.types || [];
      ...
      //(`Filtered transit stations: ${filteredResults.length} out of ${data.results.length} results`)
    }
    ...
    const results = filteredResults.map((place) => ({
      ...
      photos: place.photos
        ? place.photos.slice(0, 3).map((photo) => ({
            ...
            width: photo.width,
            height: photo.height,
            url: getPlacePhotoUrl(photo.photo_reference, 800, 600), // High quality for display
            thumbnail: getPlacePhotoUrl(photo.photo_reference, 400, 300), // Medium quality for lists
            small: getPlacePhotoUrl(photo.photo_reference, 300, 200), // Small for thumbnails
          }))
        : [],
    }));
    ...
    if (
      ...
      // //(`${type} search returned few results, trying alternative searches...`)
      ...
      for (const query of alternativeQueries) {
        try {
          ...
          const altResponse = await fetch(
            `${MAPS_API_BASE}/place/textsearch/json?${altParams}`
          );
          ...
          if (altData.status === "OK" && altData.results.length > 0) {
            ...
            if (type === "electric_vehicle_charging_station") {
              altFiltered = altData.results.filter((place) => {
                ...
                const notDuplicate = !results.some(
                  (existing) => existing.placeId === place.place_id
                );
              ...
              altFiltered = altData.results.filter((place) => {
                ...
                const isNotGenericPlace =
                  ...
                  !name.includes("mess") &&
                  !types.some(
                    (t) => t.includes("restaurant") || t.includes("store")
                  );
            ...
            const altResults = altFiltered.map((place) => ({
              ...
              photos: place.photos
                ? place.photos.slice(0, 3).map((photo) => ({
                    ...
                    width: photo.width,
                    height: photo.height,
                    url: getPlacePhotoUrl(photo.photo_reference, 400, 300), // High quality
                    thumbnail: getPlacePhotoUrl(
                      photo.photo_reference,
                      400,
                      300
                    ), // Medium quality
                    small: getPlacePhotoUrl(photo.photo_reference, 300, 200), // Small thumbnails
                  }))
                : [],
            }));
          ...
          console.error(
            `Error in alternative EV search for "${query}":`,
            error
          );
        }
      }
...
export async function getAreaTransportSummary(location, radius = 5000) {
  try {
    ...
    const summary = {
      ...
      ecoFriendlyOptions: transportOptions.filter((t) => t.ecoScore >= 4)
        .length,
      zeroEmissionOptions: transportOptions.filter((t) => t.carbonFactor === 0)
        .length,
...
function generateAreaRecommendations(transportOptions) {
  ...
  const publicTransit = transportOptions.filter(
    (t) => t.type.includes("station") || t.type.includes("transit")
  );
  const bikeOptions = transportOptions.filter((t) =>
    t.type.includes("bicycle")
  );
  ...
  try {
    ...
    if (data.status !== "OK" || !data.results.length) {
      ...
      if (fallbackCoordinates) {
        //('Using fallback coordinates for:', address)
    ...
    const fallbackCoordinates = getFallbackCoordinates(address);
    if (fallbackCoordinates) {
      //('Using fallback coordinates after error for:', address)
      return fallbackCoordinates;
    }
...
// Get Google Places photo URL with quality optimization
export function getPlacePhotoUrl(
  ...
  // Google Places API supports up to 1600px for maxwidth
  // Using higher resolution for better quality
  const width = Math.min(maxWidth, 1280);
  const height = Math.min(maxHeight, 720);

  return `${MAPS_API_BASE}/place/photo?maxwidth=${width}&maxheight=${height}&photo_reference=${photoReference}&key=${GOOGLE_MAPS_API_KEY}`;
}
...
export async function getTransportImages(transportType, location) {
  try {
    ...
    const query =
      searchQueries[transportType] || transportType.replace("_", " ");
    ...
    if (data.status === "OK" && data.results.length > 0) {
      ...
      if (place.photos && place.photos.length > 0) {
        return {
          mainImage: getPlacePhotoUrl(
            place.photos[0].photo_reference,
            800,
            600
          ), // High quality main image
          thumbnails: place.photos.slice(0, 3).map(
            (photo) => getPlacePhotoUrl(photo.photo_reference, 400, 300) // Medium quality thumbnails
          ),
          highRes: place.photos.slice(0, 3).map(
            (photo) => getPlacePhotoUrl(photo.photo_reference, 400, 300) // Maximum quality for detail views
          ),
        };
      }
    }

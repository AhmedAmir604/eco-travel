...
class AmadeusAPI {
  constructor() {
    ...
    this.token = "7OiXAuWFZPkG7SaUteBdeUGO268r";
  }
  ...
  async getLiveToken() {
    try {
      ...
      // Update the token and set expiry
  ...
  async searchHotels(cityName, checkIn, checkOut, adults = 2, radius = 25) {
    try {
      // Check if token is expired or missing, get fresh token
      ...
      try {
        ...
        // console.log(`Using Google Maps coordinates for ${cityName}:`, coordinates);
      } catch (geocodeError) {
        // console.log('Google Maps geocoding failed, using fallback coordinates');
      ...
      // console.log("Searching hotels with params:", Object.fromEntries(params));
      const response = await fetch(
        `${this.baseURL}/v1/reference-data/locations/hotels/by-geocode?${params}`,
        { headers: { 'Authorization': `Bearer ${this.token}` } }
      );

      if (response.ok) {
        const data = await response.json();
        // console.log(`Found ${data.data?.length || 0} hotels near ${cityName}`);

        if (data.data?.length > 0) {
          return await this.convertToEcoHotels(data.data.slice(0, 10), cityName);
        }
      } else {
        const errorData = await response.json().catch(() => ({}));
        console.error('Amadeus API error:', response.status, errorData);

        // If unauthorized, try to get fresh token and retry once
        if (response.status === 401) {
          // console.log('Unauthorized, getting fresh token and retrying...');
          await this.getLiveToken();
          ...
          if (retryResponse.ok) {
            ...
            // console.log(`Retry found ${retryData.data?.length || 0} hotels near ${cityName}`);

            if (retryData.data?.length > 0) {
              return await this.convertToEcoHotels(retryData.data.slice(0, 10), cityName);
            }
          }
        }
      }
  ...
  async convertToEcoHotels(hotelList, cityName) {
    const ecoFeatures = [
      'Eco-Certified', 'Energy Efficient', 'Solar Power', 'Water Conservation',
      'Organic Food', 'Recycling Program', 'Green Building', 'Local Sourcing',
      'Carbon Neutral', 'Waste Reduction', 'EV Charging'
    ];

    // Process hotels in parallel to get images
    const hotelsWithImages = await Promise.all(
      hotelList.map(async (hotel, index) => {
        const features = ecoFeatures.sort(() => 0.5 - Math.random()).slice(0, 3);
        const distance = hotel.distance ? `${hotel.distance.value.toFixed(1)} ${hotel.distance.unit}` : '';

        // Get hotel image from Google Places
        const hotelImage = await this.getHotelImage(hotel.name, cityName);

        return {
          id: hotel.hotelId,
          name: hotel.name,
          location: `${cityName}${distance ? ` (${distance} from center)` : ''}`,
  ...
  async getHotelImage(hotelName, cityName) {
    try {
      ...
      if (placeData && placeData.photos && placeData.photos.length > 0) {
        ...
        return getPlacePhotoUrl(placeData.photos[0].photo_reference, 400, 300);
      }
  ...
  getCityCoordinates(cityName) {
    ...
    return coordinates[cityName.toLowerCase().trim()] || { latitude: 40.7128, longitude: -74.0060 };
  }
  ...
  async getHotelOffers(hotelId, checkIn, checkOut, adults = 1) {
    try {
      ...
      // console.log('Fetching hotel offers with params:', Object.fromEntries(params));
      ...
      if (response.ok) {
        ...
        if (data.data && data.data.length > 0) {
          ...
          return {
            ...
            features: [
              'Eco-Certified', 'Energy Efficient', 'Solar Power', 'Water Conservation',
              'Organic Food', 'Recycling Program'
            ].sort(() => 0.5 - Math.random()).slice(0, 4),
            amenities: ['Restaurant', 'WiFi', 'Fitness Center', 'Swimming Pool'],
        ...
        const errorData = await response.json().catch(() => ({}));
        console.error('Hotel offers API error:', response.status, errorData);
        ...
        if (response.status === 401) {
          ...
          if (retryResponse.ok) {
            ...
            if (retryData.data && retryData.data.length > 0) {
              const hotelData = retryData.data[0];
              const hotelImage = await this.getHotelImage(hotelData.hotel.name, '');

...
export async function getAmadeusToken() {
  try {
    ...
    console.log("token we get is " , data.access_token);
...
export async function handleAmadeusError(response, context) {
  ...
  if (response.status === 401) {
    console.warn(`Amadeus API unauthorized for ${context}, clearing token cache`);
    // Clear token cache on 401 to force refresh
    tokenCache.token = null;
    tokenCache.expiry = null;
  }
  
  const error = new Error(`Amadeus API error (${response.status}): ${errorText}`);

    ...
    const hotelsWithImages = await Promise.all(
      hotelList.map(async (hotel, index) => {
        ...
        return {
          ...
          image: hotelImageData?.url || "/placeholder.svg",
          images: hotelImageData || { url: "/placeholder.svg" },
  ...
  async getHotelImage(hotelName, cityName) {
    try {
      ...
      if (placeData && placeData.photos && placeData.photos.length > 0) {
        // COST OPTIMIZATION: Reduced image sizes significantly
        ...
        // Return just the main image to minimize photo API costs
        return {
          url: mainImage,
        };
      }
  ...
  async searchGooglePlaces(query) {
    try {
      ...
      const params = new URLSearchParams({
        ...
        // COST OPTIMIZATION: Only request essential fields (photos, name, place_id)

...
async function generateDailyActivities(
  ...
  // Shuffle and distribute POIs across days, ensuring at least 4 per day
  const shuffledPois = [...allPois].sort(() => 0.5 - Math.random());
  const minPoisPerDay = 4;
...
async function generateDayActivities(
  ...
  try {
    ...
    if (pois.length > 0) {
      ...
      // Create at least 4 activities per day using available POIs (save evening for dining)
      const numActivities = 4; // Always ensure 4 activities per day
    ...
    // Evening activity (food/local experience) - try to find real restaurants
    try {
      const restaurants = await findRestaurantsForLocation(coordinates);

      let eveningActivity;
      if (restaurants.length > 0) {
        // Use real restaurant data
        const restaurant = restaurants[0]; // Get the best rated restaurant
        eveningActivity = {
          time: "18:00",
          type: "food",
          title: `Dinner at ${restaurant.name}`,
          description: `Enjoy ${restaurant.cuisine.toLowerCase()} at this highly-rated local restaurant${
            sustainabilityLevel === "high"
              ? " known for sustainable practices and locally-sourced ingredients"
              : ""
          }.`,
          ...
          kindsArray: restaurant.kinds
            ? restaurant.kinds.split(",").map((k) => k.trim())
            : ["foods"],
        };
      } else {
        // Fallback to generic dining activity
        eveningActivity = {
          time: "18:00",
          type: "food",
      ...
      console.error(
        "Error fetching restaurants, using fallback:",
        restaurantError
      );

      // Fallback evening activity
...
async function getPointsOfInterest(
  ...
  try {
    ...
    const categories = interests.flatMap(
      (interest) => INTEREST_CATEGORIES[interest] || ["cultural"]
    );
    ...
    for (const category of allCategories.slice(0, 5)) {
      ...
      if (response.ok) {
            ...
            data
              ...
              .map(async (poi) => {
                ...
                return {
                  ...
                  description:
                    detailedPoi?.description ||
                    generatePoiDescription(poi.name, category, poi.kinds),
                  ...
                  kindsArray: poi.kinds
                    ? poi.kinds.split(",").map((k) => k.trim())
                    : [category],
                };
              })
          );
        ...
        console.warn(
          `Failed to fetch POIs for category ${category}:`,
          response.status,
          response.statusText
        );
      }
    ...
    if (allPOIs.length < 8) {
      ...
      const broadResponse = await fetch(
        `${OPENTRIPMAP_BASE}/radius?${broadParams}`
      );
      if (broadResponse.ok) {
            ...
            broadData
              ...
              .map(async (poi) => {
                ...
                return {
                  ...
                  description:
                    detailedPoi?.description ||
                    generatePoiDescription(poi.name, "cultural", poi.kinds),
                  wikidata: detailedPoi?.wikidata || null,
                  wikipedia: detailedPoi?.wikipedia || null,
                  image: detailedPoi?.image || null,
                  address: detailedPoi?.address || null,
...
// Find restaurants for a location using OpenTripMap API
async function findRestaurantsForLocation(coordinates, radius = 1000) {
  try {
    const response = await fetch(
      `https://api.opentripmap.com/0.1/en/places/radius?radius=${radius}&lon=${coordinates.lng}&lat=${coordinates.lat}&kinds=foods&format=json&limit=20&apikey=${OPENTRIPMAP_API_KEY}`
    );

    if (!response.ok) {
      console.error(
        "Failed to fetch restaurants from OpenTripMap:",
        response.statusText
      );
      return [];
    }
    ...
    const restaurants =
      data.features?.map((feature) => ({
        name: feature.properties.name || "Local Restaurant",
        address: feature.properties.address || "Local area",
        cuisine: feature.properties.kinds?.includes("bars")
          ? "Bar & Grill"
          : feature.properties.kinds?.includes("fastfood")
          ? "Fast Food"
          : "Local Cuisine",
        rating: feature.properties.rate || 4.0,
        coordinates: {
    ...
    return restaurants
      .filter((r) => r.name !== "Local Restaurant" || restaurants.length < 3) // Prefer named restaurants
      .sort((a, b) => b.rating - a.rating || a.distance - b.distance)
      .slice(0, 5); // Return top 5 restaurants
  } catch (error) {
...
function getDiningDescription(dayNumber, sustainabilityLevel) {
  ...
  return (
    ...
    `Experience local cuisine at eco-friendly restaurants ${ecoFocus}.`
  );
}

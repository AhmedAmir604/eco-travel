  ...
  try {
    ...
    if (pois.length > 0) {
      ...
      for (let i = 0; i < numActivities; i++) {
        ...
        // Enhanced description generation with more detail
        let description = await generateEnhancedDescription(poi, activityType, sustainabilityLevel)
        ...
        activities.push({
          ...
          location: poi.location || `${poi.name} area`,
          ...
          ecoFeatures: getEcoFeaturesForActivity(activityType, poi.kinds),
  ...
  try {
    ...
    for (const category of allCategories.slice(0, 5)) { // Search up to 5 categories
      ...
      if (response.ok) {
        ...
        console.log(`pois from opentrimap for ${category}:`, data?.length || 0, 'results')

        if (Array.isArray(data) && data.length > 0) {
          const pois = data
            .filter(poi => poi.name && poi.name.trim() !== '' && poi.rate >= 1) // Filter out low-quality POIs
            .map(poi => ({
              id: poi.xid,
              ...
              description: generatePoiDescription(poi.name, category, poi.kinds),
              rating: poi.rate || 1,
    ...
    if (allPOIs.length < 8) {
      ...
      const broadParams = new URLSearchParams({
        radius: radius * 1.5, // Increase radius
        limit: 20,
        lon: coordinates.lng,
        lat: coordinates.lat,
      ...
      if (broadResponse.ok) {
        ...
        console.log('Broad search POIs:', broadData?.length || 0, 'results')

        if (Array.isArray(broadData) && broadData.length > 0) {
          const broadPois = broadData
            ...
            .map(poi => ({
              ...
              location: poi.name,
              description: generatePoiDescription(poi.name, 'cultural', poi.kinds),
              rating: poi.rate || 1,
              kinds: poi.kinds || 'interesting_places'
            }))

          allPOIs.push(...broadPois)
        }
      }
    }

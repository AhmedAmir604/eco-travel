export const calculateRouteFromUserLocation = (userLocation, destination, travelMode, directionsService, directionsRenderer, routeInfoId = 'route-info') => {
        ...
        routeInfoDiv.innerHTML = `
          <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
            <div class="flex items-center gap-4 text-sm">
              ...
              <div class="flex items-center gap-2">
                <span class="text-blue-600">🌱</span>
                <span class="font-medium">${travelMode === 'WALKING' ? '0kg CO₂' : travelMode === 'TRANSIT' ? 'Low CO₂' : 'Standard CO₂'}</span>
              </div>
            </div>
          </div>
        `
    ...
    activity: {
      url: 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(`
        <svg width="36" height="36" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg">
          ...
          <text x="18" y="23" text-anchor="middle" fill="white" font-size="12" font-weight="bold">${index !== null ? index + 1 : '?'}</text>
        </svg>
      `),
...
export const createInfoWindowContent = (type, data, userLocation, showUserLocationRoutes) => {
  if (type === 'hotel') {
    return `
  ...
  if (type === 'activity') {
    return `
      <div class="p-3 max-w-sm">
        <div class="flex items-center gap-2 mb-2">
          <span class="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-bold">${data.time}</span>
          <span class="bg-green-100 text-green-700 px-2 py-1 rounded text-xs">${data.carbonFootprint}kg CO₂</span>
          ${data.rating ? `<span class="bg-yellow-100 text-yellow-700 px-2 py-1 rounded text-xs">★ ${data.rating}/5</span>` : ''}
        </div>
        <h3 class="font-bold text-lg mb-2">${data.title}</h3>
        ${data.image ? `<img src="${data.image}" alt="${data.title}" class="w-full h-24 object-cover rounded mb-2" onerror="this.style.display='none'">` : ''}
        <p class="text-sm text-gray-600 mb-3">${data.description.substring(0, 150)}${data.description.length > 150 ? '...' : ''}</p>
        
        ${data.kindsArray && data.kindsArray.length > 0 ? `
          <div class="mb-2">
            <div class="text-xs text-gray-500 mb-1">Categories:</div>
            <div class="flex flex-wrap gap-1">
              ${data.kindsArray.slice(0, 4).map(kind => `
                <span class="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded">${kind.replace(/_/g, ' ')}</span>
              `).join('')}
              ${data.kindsArray.length > 4 ? `<span class="text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded">+${data.kindsArray.length - 4}</span>` : ''}
            </div>
          </div>
        ` : ''}
        
        <div class="flex flex-wrap gap-1 mb-2">
          ${data.ecoFeatures?.map(feature => `
